...
import {
  FetchGetReturnOrImportedGoodsResponse,
  ReturnOrImportedType,
} from 'types/Responses/FetchGetReturnOrImportedGoodsResponse';
import ListHeader from './components/ListHeader';
import ReturnGoodItem from './components/ReturnGoodItem';
...
const ReturnOfImportedGoods = () => {
  ...
  const {
    ...
    onRefresh,
    onEndReached,
  } = useDataLoader<FetchGetReturnOrImportedGoodsResponse>(
    hasReadOnlyPermission as boolean,
    OrderService.fetchGetReturnOrImportedGoods,
    filterParams,
  );
  ...
  const onAdd = useCallback(() => {
    NavigationService.pushToScreen(RootScreenID.Transaction, {
      ...
      params: {isImportProduct: false},
    });
  }, []);

  const onItemPress = useCallback((item: ReturnOrImportedType) => {
    NavigationService.pushToScreen(RootScreenID.Transaction, {
      screen: TransactionScreenID.ReturnOfImportedGoodDetails,
  ...
  const renderItem = ({
    ...
    return <ReturnGoodItem key={index} item={item} onItemPress={onItemPress} />;
  };
  ...
  const renderContent = () => {
    ...
    return (
      <CustomFlatList
        ...
        keyExtractor={(item: ReturnOrImportedType) => item.id.toString()}
        data={data}
...
export default ReturnOfImportedGoods;
