...
import BookByRoomItem from './components/BookByRoomItem';
...
const BookByRoom = () => {
  ...
  const onAdd = useCallback(() => {
    NavigationService.pushToScreen(RootScreenID.Transaction, {
      screen: TransactionScreenID.SelectRooms,
    });
  }, []);

  const onItemPress = useCallback((item: OrderItemType) => {
    NavigationService.pushToScreen(RootScreenID.Transaction, {
      screen: TransactionScreenID.BookByRoomOrderDetails,
      params: {
        id: item.id,
        code: item.code,
      },
  ...
  const renderItem = ({item, index}: {item: OrderItemType; index: number}) => {
    return (
      <BookByRoomItem index={index} item={item} onItemPress={onItemPress} />
    );
  };
